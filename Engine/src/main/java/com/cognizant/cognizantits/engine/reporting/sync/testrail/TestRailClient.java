...
public class TestRailClient {
    ...
    public static String UPDATE_RUN = "index.php?/api/v2/update_run/<runId>";
    public static String GET_RUNS = "index.php?/api/v2/get_runs/<projectId>";
    public static String GET_TESTIDS = "index.php?/api/v2/get_cases/<projectId>&suite_id=<suiteId>";
    public static String GET_RESULTID = "index.php?/api/v2/get_results_for_case/<runId>/<testId>";
    ...
    public static String ADD_ATTACHMENT_FOR_CASE = "index.php?/api/v2/add_attachment_to_result/<resultId>";
    ...
    public String getRunID(String project,String tset) {
    	try {
            String GET_RUNS_URL = GET_RUNS.replace("<projectId>", getProjectId(project));
            JSONObject res = client.Get(new URL(client.url + GET_RUNS_URL));
            JSONArray runsList = (JSONArray) res.get("array");
            for (int i = 0; i < runsList.size(); i++) {
                JSONObject run = (JSONObject) runsList.get(i);
                if (run.get("name").toString().equalsIgnoreCase(tset)) {
    ...
    public String getResultID(String runId, String testId) {
        try {
            String GET_RESULTID_URL = GET_RESULTID.replace("<runId>", runId);
            GET_RESULTID_URL=GET_RESULTID_URL.replace("<testId>", testId);
            JSONObject res = client.Get(new URL(client.url + GET_RESULTID_URL));
            JSONArray resultList = (JSONArray) res.get("array");
    ...
    public void addAttachmentForResultId(String resultId, File filename) throws MalformedURLException, Exception {
        String ADD_ATTACHMENT_FOR_CASE_URL = ADD_ATTACHMENT_FOR_CASE.replace("<resultId>", resultId);
        JSONObject res = client.post((new URL(client.url + ADD_ATTACHMENT_FOR_CASE_URL)), filename,"attachment","test");
        JSONArray resList = (JSONArray) res.get("array");
    }
    ...
    public int updateResult(int status, String testCase, String tset, String rls, String project, List<File> attach)
        ...
        String runId = getRunID(project, tset);
        if (runId.isEmpty()) {
        ...
        String resultId = getResultID(runId,testId);
        for (int i = 0; i < attach.size(); i++) {
        	addAttachmentForResultId(resultId, attach.get(i));
        }
        return 0;
    }
}
